const resize = () => {
  Array(...document.querySelectorAll(".slidee")).map(
    (e) =>
      (e.style.width = getComputedStyle(document.querySelector("header")).width)
  );
};
...
function getDataForHomePageLanding() {
  database.ref("landingText/").on("value", function (e) {
    ...
    a.classList.add("slidee"),
      r.classList.add("overlayer"),
      (document.querySelector(
        "div.container:has(header.sliderr)"
      ).style.backgroundImage = `url(${t.image})`),
      (l.innerText = t.title),
      (n.innerText = t.subTitle),
      document.querySelector("div.container:has(header.sliderr)").append(r),
      a.append(l, n),
      document.querySelector("header.sliderr").append(a);
  });
}
