...
const firebaseConfig = {
  ...
  databaseURL:
    "https://egyption-monuments-default-rtdb.europe-west1.firebasedatabase.app",
...
function allForm(e, t, a, z, from, xxx) {
  let n;
  a &&
    (((n = document.createElement("p")).innerText = "العودة↩️"),
  ...
  database.ref(`${from}/`).on("value", function (e) {
    ...
    t &&
      ...
      (loading.children[loading.children.length - 1].innerText =
        "جاري التحميل...")),
    setTimeout(() => {
      ...
      if (z) {
        for (let l in o)
          if (Object.hasOwnProperty.call(o, l)) {
            ...
            (s.innerText = i.title),
              (m.innerText = i.p),
              c.classList.add("overlayer"),
              (d.style.backgroundImage = `url(${i.image})`),
              d.append(c, s, m),
              r.append(d);
          }
        r.classList.add("galary");
      } else {
        for (let l in o)
          if (Object.hasOwnProperty.call(o, l)) {
            let i = o[l],
      ...
      a
        ? document.querySelector(`.container#${e}`).append(n, r)
        : document.querySelector(`.container#${e}`).append(r),
        t && loading.classList.remove("active");
    }, timeout);
}
onload = () => {
  setTimeout(() => {
